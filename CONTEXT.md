...
| 2026-04-08 | Eval harness complete: Added `scripts/eval.py` (single-command in-process harness), `scripts/eval_matching.py` (pure, testable finding→truth matcher with tolerant path resolution, line proximity +/-5, CWE-in-file fallback, greedy one-to-one assignment, explicit `duplicate_findings` list so overlapping rule hits aren't double-counted), `scripts/eval_report.py` (Markdown + JSON renderer), `eval-artifacts/.gitignore` (raw run dirs), and `pyyaml` to backend requirements. Harness mutates `settings.db_path` to a throwaway `eval.db` per run, calls `run_semgrep_scan` / `run_llm_review_scan` / `run_patch_review_loop` / `verify_patch` directly (no HTTP), and computes three Hunter views (Semgrep alone / LLM alone / hybrid union) with per-scanner recall bound to each truth entry's `detectable_by` tag so scanners aren't credited for or penalised against findings outside their expected set. Metrics: per-scanner P/R/F1, LLM-only contribution count, Surgeon apply rate + mean attempts + retry rate, Critic approval rate + verifier agreement + false-accept rate, end-to-end fix funnel, per-stage p50/p95 timings. Smoke-verified on Semgrep-only Hunter: `vibe-todo-app` 4/11 detected (P=1.00, R=36%), `vibe-notes-api` 9/9, `vibe-file-share` 5/9, `vibe-auth-service` 3/9. Aggregate Semgrep-alone recall 94% against its expected subset, hybrid recall 55% before LLM pass. `EVAL.md` section added to README. |

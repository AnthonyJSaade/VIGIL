...
## Recent Fixes
- **2026-04-20 — Patch pipeline end-to-end hardening (Surgeon → Critic → Verifier).** Single consolidated pass that took the patch flow from "fails on the very first `eval` finding" to "verifies cleanly across all four demo repos." Changes span backend agents, the verifier, the scanner normalizer, and the frontend pipeline UI. Landed together because they all fall out of the same root question: *"why does `patch -p1` fail in the sandbox?"*
  - **Model deprecation hardening (`backend/app/config.py`, `agents/surgeon.py`, `agents/critic.py`, `.env.example`, `README.md`).** Surgeon + Critic were pinned to `claude-sonnet-4-20250514` (deprecated). Added env-backed settings `VIGIL_SURGEON_MODEL` / `VIGIL_CRITIC_MODEL` (defaults `claude-sonnet-4-6`) so model migrations no longer require code edits; both agents now read from `settings` and record `model_used` from the same source.
  - **Surgeon now returns full patched file; backend computes the diff (`agents/surgeon.py`).** LLM-authored diffs were the dominant failure mode — hallucinated context lines (imports sections that didn't exist), missing context-line leading spaces, wrong `@@ -a,b +c,d @@` counts, and repo-prefixed paths (`a/demo-repos/…/server.js`). Refactored the Surgeon to return `{patched_content, explanation}` instead of a raw `diff`. Backend then computes the unified diff deterministically via `difflib.unified_diff` in `_compute_diff(original, patched, file_path)` with canonical `a/<path>` / `b/<path>` headers drawn straight from `finding.file_path`. `PatchProposal.diff` still carries a unified diff, so Critic, UI, HTML/ZIP export, and `apply_patch_in_temp` are unchanged; Critic continues to see only the diff (independence preserved per `architecture.mdc`). Bumped `max_tokens` to 8192 for full-file returns.
  - **Scanner normalizer emits repo-relative paths (`scanner/normalizer.py`, `verification/sandbox.py`).** Semgrep is invoked with an absolute repo path and returns absolute finding paths (e.g. `/demo-repos/vibe-todo-app/server.js`); the normalizer was passing those through, and concatenating `f"a/{file_path}"` then produced `a//demo-repos/...` (double slash) which `patch` couldn't resolve at any strip level. `normalize_findings` now resolves each finding path relative to `repo_path` (falling back to `lstrip("/")` if the finding isn't under the repo root). Verifier's post-patch rescan also passes `sandbox_repo` so `_finding_still_present` compares apples-to-apples. Net effect: `finding.file_path` is always `server.js` / `src/foo.ts`, diff headers are always `a/server.js` / `b/server.js`, `patch -p1` resolves on the first try.
  - **Verifier robustness against malformed diffs (`verification/sandbox.py`).** Even after the Surgeon-side fix, kept the defensive layer in place for any future LLM-authored patches: `_normalize_diff` prefixes bare hunk-body lines with a space and recomputes `@@` counts from the actual ±/context totals; `_detect_strip_level` picks the smallest `-pN` where every diff target resolves in the sandbox, falling back through 1/0/2/3/4; `_run_patch` passes `--batch` so `patch` fails fast instead of prompting `File to patch:` on `/dev/tty`, plus a `PATCH_TIMEOUT_SECONDS = 30` `asyncio.wait_for` safety-net mirroring `scanner/runner.py`. Malformed diffs now fail cleanly in ~20ms with the diagnostic surfaced in the `VerificationReport.details`.
  - **Frontend pipeline UX (`frontend/components/vigil/patch-pipeline.tsx`).** Two regressions that only appeared after a failed verification: (1) **Regenerate Patch** selected the *first* approved attempt for verification, which already had a `/verification` row, so the backend returned 409 and the UI showed a red "Failed to start verification"; added a `findLatestApproved` helper so verify always targets the newest approved patch. (2) The generic error-state **Retry** button always called `startPatch`, so retrying a failed *verification* restarted Surgeon from scratch; tracked `lastAction` ("patch" | "verify") and routed Retry accordingly. Error messages from the backend now render verbatim instead of a generic "Something went wrong."
  - **Verification semantics preserved, not rubber-stamped.** `verify_patch` still: copies the repo to a temp sandbox, runs real `patch -p1`, re-runs Semgrep with the same ruleset, and calls `_finding_still_present` (same `rule_id`, same `file_path`, ±5-line overlap) — `scanner_rerun_clean=True` is set only when the original rule no longer fires. The UI's green "Fix Verified" card is gated on that boolean end-to-end.
...
| `backend/app/config.py` | Settings via pydantic-settings (VIGIL_ env prefix): anthropic_api_key, surgeon_model, critic_model, db_path, demo_repos_path |
...
| `.env.example` | Template for VIGIL_ANTHROPIC_API_KEY + optional VIGIL_SURGEON_MODEL / VIGIL_CRITIC_MODEL overrides |
...
| 2026-04-20 | Patch pipeline end-to-end hardening: consolidated 5 patches that took Surgeon→Critic→Verifier from "fails on first eval audit" to "verifies cleanly across all 4 demo repos." Model IDs moved to env (`VIGIL_SURGEON_MODEL`, `VIGIL_CRITIC_MODEL`, default `claude-sonnet-4-6`); Surgeon now returns full patched file and backend computes the diff via `difflib.unified_diff`; scanner normalizer emits repo-relative paths so diff headers are always `a/server.js`; verifier keeps defensive `_normalize_diff` / `_detect_strip_level` / `--batch` / 30s timeout; frontend pipeline tracks `lastAction` and `findLatestApproved` so Regenerate + Retry behave correctly. Verification semantics unchanged — green "Fix Verified" still gated on `scanner_rerun_clean` (same `rule_id` + `file_path` + ±5-line overlap no longer firing in Semgrep rerun). See **Recent Fixes** for details. |
